...
class UnsplashApiProvider {
    ...
    fun getPhotoDetails(id: String, cb: UnsplashResult) {
        retrofit.getPhotoDetails(id = id).enqueue(object : Callback<PhotoDetails> {
            override fun onResponse(call: Call<PhotoDetails>, response: Response<PhotoDetails>) {
                if (response.isSuccessful && response.body() != null) {
                    Log.d(TAG, "getPhotoDetails | response: ${response.body()}")
            ...
            override fun onFailure(call: Call<PhotoDetails>, t: Throwable) {
                Log.e(TAG, "getPhotoDetails | error loading images", t)
                cb.onDataFetchedFailed()
            }
        })
    }
